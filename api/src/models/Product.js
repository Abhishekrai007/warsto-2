...
const ProductSchema = new Schema(
  {
    ...
    attributes: {
      ...
      woodwork: {
        carcassMaterial: [
          { type: String, enum: ["MR", "BWR", "BWP", "HDRHMR"] },
        ],
        ...
        shutterFinish: {
          type: String,
          enum: ["Laminate", "Acrylic", "PU", "RPU", "VENR", "GLX"],
        },
      ...
      brand: {
        type: String,
        enum: ["Merino", "Greenlam", "Senosan", "Asian Paints", "SaintGob"],
      },
      configuration: String,
    },
    hardware: {
      channels: [
        { type: String, enum: ["Hettich", "Haffle", "ebco", "heppo", "NA"] },
      ],
      hinges: [
        { type: String, enum: ["Hettich", "Haffle", "ebco", "heppo", "NA"] },
      ],
      hRodsAndAccessories: [
        {
          type: String,
          enum: ["Hettich", "Haffle", "Ebco", "Heppo", "Onyx", "NA"],
        },
      ],
    },
    designer: {
...
ProductSchema.methods.updateReviewStats = async function (
  ...
  const totalRatings = Object.values(result.reviews.ratingDistribution).reduce(
    (a, b) => a + b,
    0
  );
  const weightedSum = Object.entries(result.reviews.ratingDistribution).reduce(
